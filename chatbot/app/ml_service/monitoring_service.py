...
# Reference distributions from ML model training datasets
...
class MLMonitoringService:
    """
    Lightweight telemetry & operational health monitoring service for trained ML models.
    Tracks inference counts, latencies (ms), errors, validation failures, latest predictions,
    and calculates statistical input drift against training baselines.
    """
    ...
    def record_inference_start(self, model_key: str) -> float:
        """Returns current high-resolution performance counter timestamp."""
    ...
    def record_inference_success(
        ...
        """Calculates inference latency and updates telemetry metrics."""
    ...
    def record_inference_error(self, model_key: str, error_msg: str) -> None:
        """Records an inference execution failure."""
        m = self._metrics.setdefault(model_key, self._init_model_metric(model_key))
        m["error_count"] += 1
    ...
    def check_input_drift(self, model_key: str) -> Dict[str, Any]:
        """
        Calculates statistical input drift for a model based on Z-score bounds against reference training data.
        Returns 'Monitoring baseline unavailable' if insufficient samples (<5) exist.
        """
    ...
    def get_monitoring_report(self) -> Dict[str, Any]:
        """Returns a comprehensive telemetry & health report for all registered models."""
        report = {}
        for model_key in EXPECTED_MODELS:
    ...
    def get_system_alerts(self) -> List[Dict[str, Any]]:
        """Generates real alerts based on model status, latency, error count, and drift."""
        alerts = []
