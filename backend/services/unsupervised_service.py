...
class UnsupervisedService:
    ...
    def detect_surge(self, state: HospitalState) -> SurgeDetectionResponse:
        """Detect anomalous arrival surges and operational strain using DBSCAN distance logic."""
        df_feat = self._prepare_features(state)
        scaler = artifact_loader.unsupervised_scaler
        kmeans = artifact_loader.kmeans_model
        ...
        # Anomaly threshold check (distance > 1.2 or arrival rate significantly exceeding baseline)
        ...
        return SurgeDetectionResponse(
            ...
            model_name="DBSCAN Anomaly Detector"
        )
